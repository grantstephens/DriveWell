 ...
 * directly, because expo-sqlite has no Node build. Without this seam the
 * SQLite store could only be exercised on a device, and the behavioural
 * contract suite would have to run against a mock instead of real SQLite.
 * Implemented by expoSqlite.ts on device and by nodeSqlite.ts in tests.
 */
